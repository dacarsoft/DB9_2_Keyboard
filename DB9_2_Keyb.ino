...
const uint8_t DB9_CYCLES_PRESS = 5; // DEBOUNCE CYCLES TO VALIDATE PRESS
...
void loop() {
  ...
  joystickRead(DB9_1_PINS, DB9_1_TOTALPINS, DB9_1_SELECT, DB9_1_STATUS, DB9_1_PRESSCOUNT, DB9_1_MAP, DB9_1_MAP_ACTIVE, 1);
  joystickRead(DB9_2_PINS, DB9_2_TOTALPINS, DB9_2_SELECT, DB9_2_STATUS, DB9_2_PRESSCOUNT, DB9_2_MAP, DB9_2_MAP_ACTIVE, 2);
...
uint8_t sendPS2keypress(char keycode) {
  ...
  if (keycode == PS2dev::UP_ARROW || keycode == PS2dev::DOWN_ARROW || keycode == PS2dev::LEFT_ARROW || keycode == PS2dev::RIGHT_ARROW || keycode == PS2dev::RIGHT_ALT) {
    while (digitalRead(PS2_PINS[0]) == LOW) {}
    delayMicroseconds(5);
    while (digitalRead(PS2_PINS[0]) == LOW) {}
...
void joystickRead(const uint8_t JOYSTICK_PINS[6], const uint8_t JOYSTICK_TOTALPINS, const uint8_t JOYSTICK_SELECT, uint8_t JOYSTICK_STATUS[12], uint8_t JOYSTICK_PRESSCOUNT[12], const char JOYSTICK_MAP[12][12], uint8_t JOYSTICK_MAP_ACTIVE, uint8_t JOYSTICK_INDEX) {
  ...
  for ( uint8_t i = 0; i < JOYSTICK_TOTALPINS; i++ ) {
    if (digitalRead(JOYSTICK_PINS[i]) == LOW) {
      if (JOYSTICK_PRESSCOUNT[i] < DB9_CYCLES_PRESS) {
        ...
        while (sendPS2keypress(JOYSTICK_MAP[JOYSTICK_MAP_ACTIVE][i]) != 0) {}
    ...
    for ( uint8_t i = 4; i < JOYSTICK_TOTALPINS; i++ ) {
      if (digitalRead(JOYSTICK_PINS[i]) == LOW) {
        if (JOYSTICK_PRESSCOUNT[i+2] < DB9_CYCLES_PRESS) {
          ...
          DB9_CYCLES_WAITCOUNT = DB9_CYCLES_WAIT;
          while (sendPS2keypress(JOYSTICK_MAP[JOYSTICK_MAP_ACTIVE][i+2]) != 0) {}
  ...
  for ( uint8_t i = 0; i < 4; i++ ) {
    if (digitalRead(JOYSTICK_PINS[i]) == LOW) {
      if (JOYSTICK_PRESSCOUNT[i+8] < DB9_CYCLES_PRESS) {
        ...
        DB9_CYCLES_WAITCOUNT = DB9_CYCLES_WAIT;
        while (sendPS2keypress(JOYSTICK_MAP[JOYSTICK_MAP_ACTIVE][i+8]) != 0) {}
      ...
      if ((JOYSTICK_PRESSCOUNT[i+8] > 0) && (DB9_CYCLES_WAITCOUNT == 0)) {
        ...
        if ((JOYSTICK_STATUS[i+8] == 1) && (JOYSTICK_PRESSCOUNT[i+8] < 3)) {
          DB9_CYCLES_WAITCOUNT = DB9_CYCLES_WAIT;
          while (sendPS2keyrelease(JOYSTICK_MAP[JOYSTICK_MAP_ACTIVE][i+8]) != 0) {}
